...
theQuantileFunction <- qarb(do.call(unmixedDist, c(list(n=10000), distributionParameters)))
...
for (i in 1:simIterations){
  ...
  trueTweedieCorrectionInfo <- tweediesFormulaOracle(X=X, W=trueW, U=trueWTilde %*% solve(sigmaTildeMinusHalf), Sigma_gamma=Sigma_gamma, 
                                                  functionalBasis=diag(rep(1, dim(Sigma_gamma)[1])), gridStep = .001, 
...
unimxedScoreFunctionCoordinateExample <- cbind.data.frame(
    c(rep("FastICA", n_curves), rep("Our ICA", n_curves)),
    c(WAlternateICATheta[coordinateForPlotting, ],WOurMethodTheta[coordinateForPlotting, ]),
...
distributionDirectory <- paste(c("", "Mixture")[any(lengths(distributionParameters) > 1)+1], 
