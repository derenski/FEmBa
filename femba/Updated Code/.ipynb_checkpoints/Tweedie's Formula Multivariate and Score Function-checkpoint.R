...
scoreFunctionWithICA <- function(thetaMatrix, sigmaTilde, UInit, lambdaGrid=10^seq(-6, 6, 1), numKnots=5, 
  ...
  risk_t <- evaluateRisk(thetas=X, unmixedScoreFunction=psi_tVector, SigmaTilde=sigmaTilde,
                        W=Wt)
  ...
  for (iterator in 1:algorithmSpecs["max_iterations"]){
    ...
    ### The updated Ut
    UtPlusOne <- unmixingMatrixUpdate(thetaTildeMatrix= sigmaTildeMinusHalf %*% 
                                        thetasSigmaTildeCov, UInit=Ut, 
    ...
    risk_tPlusOne <- evaluateRisk(thetas=X, unmixedScoreFunction=psi_tPlusOneVector,
...
unmixingMatrixUpdate <- function(thetaTildeMatrix, UInit, 
                                 scoreFunctionVectorInit, minIterationsU=10, maxIterationsU=100,
                                 minChangeU=1e-06){

  ### The value of the risk
  rHat <- function(uRisk, ourScoreVector, thetaTildeMatrix){ 
    
    uncoupledThetaTildesUrisk <- uRisk %*% thetaTildeMatrix
    
    innerTerms <- (((ourScoreVector( uncoupledThetaTildesUrisk, derivativeOrder=0))^2)+
      2*ourScoreVector(uncoupledThetaTildesUrisk, derivativeOrder=1))
    
    return(sum(rowMeans(innerTerms, na.rm=T)))
    
  }
  
  ### The gradient of the risk
  nablaRHat <- function(uRisk, thetaTildeMatrix, scoreFunctionVector){
    
    uncoupledThetaTildesUrisk <- uRisk %*% thetaTildeMatrix
    
    multiplyConstants <- (2*scoreFunctionVector(uncoupledThetaTildesUrisk, derivativeOrder=0)
                          *scoreFunctionVector(uncoupledThetaTildesUrisk, derivativeOrder=1)
    + 2*scoreFunctionVector(uncoupledThetaTildesUrisk, derivativeOrder=2))/dim(uncoupledThetaTildesUrisk)[2]
      
    nablaValues <- multiplyConstants %*% t(thetaTildeMatrix)
    
    ### SHOULD THIS BE TRANSPOSED? (I don't believe so)
    return(nablaValues)
  ...
  for (iteratorU in 1:maxIterationsU){
    ...
    ### Gradient of risk
    nablaRHat_t <- nablaRHat(uRisk=UtMinus1, thetaTildeMatrix=thetaTildeMatrix, scoreFunctionVector)
    ...
    riskSatisfactionTerms <- function(x, y){
      ...
      return(c(rHat(uRisk=x, ourScoreVector=scoreFunctionVector, thetaTildeMatrix=thetaTildeMatrix), 
               rHat(uRisk=UtMinus1, ourScoreVector=scoreFunctionVector,thetaTildeMatrix=thetaTildeMatrix)- 
    ...
    dist <- (rHat(uRisk=UtMinus1, ourScoreVector=scoreFunctionVector,thetaTildeMatrix=thetaTildeMatrix)-
                   rHat(uRisk=Ut, ourScoreVector=scoreFunctionVector,  
                thetaTildeMatrix))/abs(rHat(uRisk=UtMinus1, ourScoreVector=scoreFunctionVector,
                                            thetaTildeMatrix=thetaTildeMatrix))
