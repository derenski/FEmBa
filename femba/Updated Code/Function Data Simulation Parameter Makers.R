...
qarb <- function(emp_dist){
  ...
  quantile_func <- function(p){
    ...
    initial_dist <- quantile(emp_dist, p)
...
curve_generator <- function(n_obs, Sigma_mu, Sigma_gamma, SNR, sigma_e, unmixedCoordinateDist,
                            times, ...){
  ...
  ICAmodelDataMaker <- function(n, p, quantile_func, covMat){
    ...
    ### True U in so(p)
    UTrue <- gramSchmidt(array(rnorm(dim(XUnmixed )[1]^2), dim=rep(dim(XUnmixed )[1], 2)))$Q 
  ...
  basis_gamma <- bSpline(times, degree = min(p_gamma,3), df = p_gamma, intercept = T)
  
  raw_snr <- (norm(Sigma_mu,type = 'F')
              /norm(Sigma_gamma, type = 'F'))
  
  muModelData <- ICAmodelDataMaker(n=n_obs, p=p_mu, quantile_func=unmixedCoordinateDist, covMat=Sigma_mu)
  ...
  specialMat <- sqrt(diag(1/diag(cov(t(mus)))))
  
  ### Makes Cov(theta)=Cor(theta)
  mus <- specialMat %*% mus
  
  cov_gamma <- (raw_snr/SNR)*Sigma_gamma
  
  gammas <- norta(n_obs=n_obs, corr_mat=cov_gamma, distribution=qnorm)
  ...
  final_output <- list(mus, basis_mu, thetas, Wtheta, U, basis_gamma, gammas, cov_gamma, e_noise)
  
  names(final_output) <- c("mu", "S_mu", "theta", "W_theta", "U", "S_gamma", "gamma", "cov_gamma", "white_noise")
